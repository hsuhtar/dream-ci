...
router.get("/cinemadetail/:id", async function(req,res){
  ...
  const movies = await Movies.find({ endDate: { $gte: Date.now() }, cinemaId: req.params.id, });
  console.log(movies);
...
router.post("/checkingSeats", async function(req,res){
  const seats = await Seats.find({cinemaId: req.body.cinemaId, screenNo: req.body.screen}).sort({
    row: 1,
    no: 1,
    screenNo: 1,
  });
